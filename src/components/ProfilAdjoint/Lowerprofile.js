...
const Lowerprofile = () => {
    ...
    const getAdjoint = async () => {
        const token = localStorage.getItem("token")
        if(token) {
            const data = await axios.get("http://localhost:3000/api/adjointParat/getOne", {
                headers:{
    ...
    const handleUpdate = async (body) => {
        try {
          const data = await axios.put(`http://localhost:3000/api/adjointParat/update/${user.id}`,body)
        } catch (error) {
          console.log(error);
        }
    }
    ...
    return (
      <div className='lowerProfile' style={{height:'100%'}}>
          <SideBarr/>
