...
const LowerProfileAdmin = () => {
    ...
    const getAdmin = async () => {
        ...
        if(token) {
            const data = await axios.get("http://localhost:3000/api/admin/getOne", {
                headers:{
    ...
    const handleUpdate = async (body) => {
        try {
          const data = await axios.put(`http://localhost:3000/api/admin/update/${user.id}`,body)
        } catch (error) {
          console.log(error);
        }
    }
...
export default LowerProfileAdmin